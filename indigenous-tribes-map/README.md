...
- Triggers on pushes to `main` (and manual runs). It builds via `pnpm dlx @cloudflare/next-on-pages@latest build` then publishes `.vercel/output/static` with `cloudflare/pages-action@v1`.
